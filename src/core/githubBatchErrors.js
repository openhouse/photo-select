...
export function batchFailureMessage(diagnostic) {
  ...
  }[code]||'See the private Batch receipt for details.';
  return `GitHub Batch curation held (${code}${diagnostic?.status?`, HTTP ${diagnostic.status}`:''}). ${guidance}`;
}
