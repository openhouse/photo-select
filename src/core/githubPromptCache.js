...
export function cacheGithubRequest(request,{brief,briefTokens,serviceTier}) {
  ...
  const stable={...request,...(serviceTier?{service_tier:serviceTier}:{}),instructions:GITHUB_CURATION_INSTRUCTIONS,text:{...request.text,format:{...request.text.format,schema}},
  ...
  const prompt_cache_key='photo-select:github-v2:'+sha256(stable).slice(0,32);
