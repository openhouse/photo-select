  ...
  while (true) {
    try {
      ...
      if (
        ...
        let rsp = await openai.responses.create(baseOpts, { headers: { 'Idempotency-Key': crypto.randomUUID() } });
        let { text } = await extractTextWithLogging(rsp);
        if (!text.trim()) {
          console.warn("⚠️ Empty text; retrying with minimal reasoning…");
          rsp = await openai.responses.create(
      ...
      console.warn(
        `retryable error (${status}); retrying in ${Math.max(wait, retryAfter)}ms…`
      );
...
export function parseReply(text, allFiles, meta = {}) {
  ...
  const preMinutes = extractBlock(text, "MINUTES", "=== DECISIONS_JSON ===");
  if (preMinutes) {
    for (const raw of preMinutes.split("\n")) {
      const line = raw.replace(/^\s*•?\s*/, "").trim();
      const m = line.match(/^([^:]+):\s*(.+)$/);
      if (m) minutes.push(`${m[1].trim()}: ${m[2].trim()}`);
    }
  }
