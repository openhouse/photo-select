...
export function schemaForBatch(
  used,
  curators = [],
  minutesMin = 3,
  minutesMax = 12,
    ...
    try {
      ...
      if (isGpt5) {
        ...
        if (!text.trim()) {
          ...
          const headers2 = { 'Idempotency-Key': crypto.randomUUID() };
...
export function parseReply(text, allFiles, meta = {}) {
  ...
  let parsed = false;
  let minutesText = body;

  // 0) Preferred path: strict block
  const startMarker = '=== DECISIONS_JSON ===';
  const endMarker = '=== END ===';
  const block = extractBlock(text, startMarker, endMarker);
  if (block) {
    try {
      ...
      if (Array.isArray(obj.decisions)) {
        for (const d of obj.decisions) {
          ...
          const base = String(d.filename || '').trim();
          if (!base) continue;
          const f = allFiles.find((p) => path.basename(p) === base);
          if (!f) continue;
          const choice = String(d.decision || '').toLowerCase();
    ...
    if (i !== -1 && j !== -1) minutesText = text.slice(0, i);
  }
...
if (!parsed) {
  ...
  if (salvage.length) {
    ...
    if (keep.size + aside.size > 0) parsed = true;
  }

  if (minutes.length === 0 && minutesText) {
    for (const raw of minutesText.split('\n')) {
      const line = raw.trim();
      const tm = line.replace(/^[\u2022*\-]\s*/, '').match(/^([^:]+):\s*(.+)$/);
      if (tm) minutes.push(`${tm[1].trim()}: ${tm[2].trim()}`);
    }
  }
