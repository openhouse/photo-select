          ...
          while (true) {
            ...
            await batchStore.run({ batch: idx }, async () => {
              try {
                ...
                const SMALL = Number(
                  process.env.PHOTO_SELECT_SMALL_BATCH_THRESHOLD || 10
                );
                ...
                if (keep.length + aside.length === 0) {
                  ...
                  if (attempts <= maxStreak) {
                    ...
                    const repairReply = await provider.chat({
                      prompt: repair,
                      images: batch,
                      model,
                      curators,
                    ...
                    const parsed = parseReply(repairReply, batch, {
                      model,
                      verbosity: 'low',
                      reasoningEffort: 'low',
                    });
                    if (parsed.keep.length + parsed.aside.length > 0) {
                      keep = parsed.keep;
                      aside = parsed.aside;
