...
it('completes 84 batches with twenty workers despite isolated cache misses and late minutes warnings',async()=>{
 const f=await setup(420),context='A "quoted" line.\n'.repeat(850);
 const result=await f.run({context,curators:['Base'],workers:20,minutesVariation:true,cacheMisses:[7,70]});
 expect(result.code,result.stderr).toBe(0);
 expect(await jpgs(f.source)).toEqual([]);
 expect(await jpgs(path.join(f.source,'_keep'))).toHaveLength(420);
 expect(await jpgs(path.join(f.source,'_aside'))).toHaveLength(420);
 const calls=(await fs.readFile(f.calls,'utf8')).trim().split('\n').map(JSON.parse);
 expect(calls).toHaveLength(84); // Neither cache recovery nor minute warnings repeat paid work.
 const audit=path.join(f.audit,(await fs.readdir(f.audit))[0]);
 ...
 const recoveries=records.filter(r=>r.attempts[0].response._photoSelectCache.role==='recovery');
 expect(recoveries).toHaveLength(2);
 expect(recoveries.every(r=>r.attempts[0].response._photoSelectCache.recoveryAttempt===1&&r.attempts[0].response._photoSelectCache.verified)).toBe(true);
 expect(records.filter(r=>r.attempts[0].response._photoSelectCache.role==='reader'&&!r.attempts[0].response._photoSelectCache.verified)).toHaveLength(2);
 expect(result.stdout+result.stderr).toContain('recovery confirmed');
