...
import {originalPromptSource, originalPromptSha256, renderOriginalPrompt, withGithubExploration} from './helpers/originalPrompt.js';
...
it('pins the original instructions independently of the current template and renderer', async () => {
  ...
  const approvedSection = '{{#if hasGithubLinks}}\nTo understand the situation more fully, explore our team’s knowledge wiki graph on GitHub.\n\n{{/if}}\n';
  expect((await fs.readFile(DEFAULT_PROMPT_PATH, 'utf8')).replace(approvedSection, '')).toBe(original);
});
...
const cases = [
  ...
  {name: 'literal Unicode and template-like context', hasGithubLinks: true, context: 'Café & <neighbors> {{curators}}\r\nhttps://github.com/fixture/private'},
  {name: 'cached GitHub context', hasGithubLinks: true, context: longContext + 'https://github.com/fixture/private'},
...
it.each(cases)('preserves pre-PR instructions with only the authorized addition: $name', async ({name, context, hasGithubLinks, ...notes}) => {
  ...
  if (hasGithubLinks) expected.prompt = withGithubExploration(expected.prompt);
  ...
  if (context.startsWith(longContext)) expect(request.prompt_cache_key).toMatch(/^photo-select:github-v3:/);
