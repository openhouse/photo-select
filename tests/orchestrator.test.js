...
describe("triageDirectory", () => {
  ...
  it("repairs zero-decision batch with decisions-only prompt", async () => {
    chatCompletion
      .mockResolvedValueOnce('no decisions')
      .mockResolvedValueOnce(
        '=== DECISIONS_JSON ==={"decisions":[{"filename":"1.jpg","decision":"keep","reason":""}]}=== END ==='
      );
    ...
    expect(secondCall.prompt).toMatch(/Return only the block below\. No minutes, no commentary./);
    await expect(
      fs.stat(path.join(tmpDir, '_keep', '1.jpg'))
    ).resolves.toBeTruthy();
  });
