...
When `--field-notes` is enabled, the tool initializes a git repository in the target directory if one is absent and commits each notebook update using the model's commit message.
...
`--parallel` starts multiple batches at once and waits for all of them to finish. This maximises throughput when every batch takes a similar amount of time but can spike CPU and memory usage.

`--workers` keeps a fixed number of workers busy and starts a new batch as soon as one finishes. It offers steadier resource usage and prints an ETA as batches complete.

### Requeue

Files skipped by a worker are requeued and handled by the next available worker so each level resolves fully before recursion continues.
...
fences and guarantees parseable output. The CLI allows up to 8192 tokens in
each reply (see `MAX_RESPONSE_TOKENS` in `src/chatClient.js`) so the minutes
and JSON decision block are returned in full.
