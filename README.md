...
6. If `--field-notes` is enabled, apply the `field_notes_diff` output to `field-notes.md` under the level snapshot directory (e.g. `_level-001/field-notes.md`). Each recursion level starts with a blank file instead of copying the parent. The diff should use standard unified patch headers—`--- a/field-notes.md`, `+++ b/field-notes.md`, and a numeric hunk header such as `@@ -0,0 +1,4 @@` for a new file—so it can be applied automatically.
